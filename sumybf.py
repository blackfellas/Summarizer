...
def main():
    ...
    path_to_db = os.path.join(path_to_cfg, 'summary.db')
    ...
    for sub in subreddits:
        ...
        length = sub[1]
        #convert blacklist to lower case list
        blist = sub[2].split('\n')
        language = sub[3]
        last_run = sub[5]
        new_run = last_run
        ...
        try:
            submissions = current_sub.get_new(limit=15)
        ...
        last_run = new_run    
        cur.execute("update summarize set last_run = ? where subreddit = ?", (last_run, sub[0]))
        con.commit()
